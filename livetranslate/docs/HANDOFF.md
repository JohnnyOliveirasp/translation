...
- ~~**BUG a corrigir (pedido do Johnny, 23/09)**~~ **CORRIGIDO 23/09 (§30)**: na aba Settings dá para mudar o nome da igreja, mas **não o link do ouvinte (slug)**. Precisa de campo editável com validação (único, minúsculas, sem espaço), redirect ou aviso de que QR/cartaz antigos param de funcionar, e manter `livekit_room` como está. Hoje só via SQL (feito para a igreja 3: `new-wine-community-church`; `livekit_room` continua `church-churchchrist-test`, funciona).
...
- **Pendências**: OK do Pastor Marcus para anúncio pago (criativo 05); Stripe BR ainda pausado (rever 24–25/09); Perfil do Google em verificação; FastCloner (Search Console + marca) fora do LiveTranslate.
- **23/09 noite — contas de mídia: NADA criado ainda.** Checagem feita no Meta do Johnny (conta pessoal johnny.oliveirasp@gmail.com, decisão dele): portfólio **JC Business Solutions USA CORP** (0 contas de anúncios, 1 Página da JC, 2 Instagrams pessoais) é onde o LiveTranslate entra; conta pessoal "lsc-7.16-JF-4" está restrita desde 07/2022 (criada pelo clone da conta dele, endereço Bangladesh, saldo zero — **ignorar, não é dívida**). Próxima sessão = criar junto: Página "LiveTranslate" → conta de anúncios USD/NY → Instagram @livetranslate.church → YouTube → Pixel. Avatar/capa/banner prontos em `_videos/CRIATIVOS-PRONTOS/perfil-redes/`. Detalhes na memória `midias-sociais-setup`.

## 30. LINK EDITÁVEL + GUIA PT PARA IGREJAS + TEXTOS ALINHADOS (23/09/2026, tarde)
- **Link (slug) editável** em Configurações → "Link dos ouvintes" + "Trocar link" (confirmação). Migration **0013** (aplicada via MCP): tabela `church_slug_aliases` (o link antigo vira apelido), RPC `change_church_slug(church, novo)` (admin da igreja ou plataforma; valida formato, reservados, duplicidade inclusive contra apelidos; recusa com culto aberto <12h: `service live`), RPC anônima `resolve_church_slug(slug)`, gatilho que impede cadastrar igreja nova com link que é apelido de outra (erro 23505 → "Esse link já está em uso"). `livekit_room` NÃO muda.
  - Ouvinte `/{link-antigo}` → redireciona para o atual (QR impresso segue funcionando). `/broadcast/{link-antigo}` → redireciona se o usuário é membro.
  - API `/api/sermons` lista e baixa PDFs da pasta atual E das pastas dos links antigos (`slugAliases` em tenant.js).
  - Igreja 3: apelido `churchchrist-test` cadastrado (a troca de 23/09 foi por SQL).
  - Cadastro: dava para digitar hífen no link? NÃO — o `slugify` a cada tecla cortava o hífen final. Agora `slugTyping` enquanto digita e `slugify` ao enviar.
- **Textos**: "Primeiro domingo grátis" → "Primeiro mês grátis" (en/es/pt, landing + título do cadastro); PDF do sermão sem a promessa de resumo/perguntas e de aprovação do pastor; orador no cadastro começa no idioma da interface (pt → Português); exemplo do link traduzido; rótulo "Role" → Função/Función.
